...
class DashboardStaticTests(unittest.TestCase):
    ...
    def test_workflow_maps_requested_report_groups(self):
        ...
        self.assertIn('report_group:', workflow)
        self.assertIn('EVENT_REPORT_GROUP: ${{ github.event.inputs.report_group }}', workflow)
        self.assertIn('REPORTS="$EVENT_REPORT_GROUP"', workflow)
    ...
    def test_daily_memo_rolls_to_next_day_and_clear_buttons_exist(self):
        ...
        self.assertIn('const MEMO_DRAFT_KEY = "krxmarket:memo:unsyncedDraft"', app)
        self.assertIn("function persistMemoDraft", app)
        self.assertIn("function loadMemoDraft", app)
        self.assertIn("function clearMemoDraft", app)
        self.assertIn("function markMemoAuthIssue", app)
        self.assertIn("let memoSaveInFlight = false", app)
        self.assertIn("let memoSaveQueued = false", app)
        self.assertIn("function memoDraftSnapshot", app)
        self.assertIn("async function flushMemoSave", app)
        self.assertIn("await fetchMemoFileMetadata(false)", app)
        self.assertIn("attempt < 4", app)
        self.assertIn("동기화 충돌 조정 중", app)
        self.assertIn("localStorage.getItem(MEMO_DRAFT_KEY) === snapshot", app)
